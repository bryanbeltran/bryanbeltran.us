...
date: '2026-07-19'
summary: 'MV3 capture architecture from production customer-care tooling — CDP, redaction, service worker constraints, and why nothing leaves the browser.'
...
I built a Chrome extension at work to replace that loop on a customer care platform: start a session, browse normally, stop and export a local ZIP with network logs, console output, browser metadata, and an offline HTML report. No upload. No telemetry. Consent required before capture starts.

This post covers the MV3 architecture behind that production tooling — not my open-source [Browser Listener](/projects) side project, which focuses on local Facebook session capture. The patterns overlap; the product scope does not.
...
On a customer care platform, the extension replaced a manual DevTools checklist for internal triage workflows. Agents and engineers get a consistent repro format; engineering spends less time asking for missing artifacts.
